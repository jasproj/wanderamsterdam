#!/usr/bin/env node
/**
 * wanderamsterdam v5.2 dry-run price extraction.
 *
 * Pulls every tour with `price === null` from tours-data.json (~233),
 ...
 *
 * Output:
 *   - scripts-staging/v52-ams-dryrun-raw.json (per-tour records)
 *   - scripts-staging/v52-ams-dryrun.md (human-readable report)
 *
 * Flag: --dry-run-only  (required; refuses to run otherwise)
 *
 * Usage:
 *   node scripts-staging/run-v52-ams-dryrun.js --dry-run-only
 *   node scripts-staging/run-v52-ams-dryrun.js --dry-run-only --limit 20  # quick smoke
 */
...
const REPORT_FILE = 'scripts-staging/v52-ams-dryrun.md';
...
(async () => {
  ...
  console.log(`wanderamsterdam null-price tours (EUR) to evaluate: ${targets.length}`);
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    try {
      ...
      const dollars = [...compact.matchAll(new RegExp('(?:€|EUR\\\\s?)\\\\s*\\\\d+(?:[.,]\\\\d{3})*(?:[.,]\\\\d{2})?', 'g'))].map(x => x[0]).slice(0, 12);
  ...
  const lines = [];
  lines.push('# wanderamsterdam v5.2 Dry-Run Report (EUR) — null-price tour re-extraction');
  lines.push('');
  lines.push(`**Generated:** ${new Date().toISOString()}`);
  lines.push(`**Branch:** \`feat/ams-v52-price-extraction\``);
  lines.push(`**Mode:** \`--dry-run-only\` (no writes to tours-data.json)`);
  lines.push('');
  lines.push('## 1. Inputs');
  lines.push('');
  lines.push(`- wanderamsterdam total tours: 500`);
  ...
  lines.push(`**Net effect if applied --live:** ${promoTotal} tours flip from "Check availability" → "From $X" (${(promoTotal/targets.length*100).toFixed(1)}% of the 233). ${stayTotal} stay hidden.`);
  lines.push('');
