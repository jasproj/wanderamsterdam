// s53-wams-schema-gate: three-state census + both-directions proof.
...
// real emitting population (loadTours() draw pool -- app.js:92 -- further
// gated by generateTourSchema's own emitPrice predicate, the rows that emit
// a bare Offer.price TODAY), then asserts:
...
// Prints counts + euro face value per state and three named fixture firings.
// usage: node census.mjs <app.js> <app-main-baseline.js> <tours-data.json>
// (app-main-baseline.js is not live code; regenerate with
//  git show origin/main:app.js > scripts/evidence/s53-wams-schema-gate/app-main-baseline.js)
import fs from 'fs';
import vm from 'vm';
...
// The loadTours() draw-pool filter, verbatim (app.js:92), further narrowed to
// generateTourSchema's own emitPrice gate (app.js's Number.isFinite(price) &&
// priceConfidence !== 'low') -- the rows that emit a bare Offer.price TODAY,
// and therefore the population this gate concerns.
const drawPool = rows.filter((t) => t.status !== 'inactive' && !t.bookingDead);
const pool = drawPool.filter((t) => Number.isFinite(t.price) && t.priceConfidence !== 'low');
    ...
    if (state === 'per-person') {
        ...
        if (cardUnit && cur.classifyUnitText(cardUnit) !== 'per-person') {
            ...
            if (!spec) err(`pk ${t.pk}: state-2a missing priceSpecification`);
                ...
                if (spec.priceCurrency !== 'EUR') err(`pk ${t.pk}: spec priceCurrency "${spec.priceCurrency}" != EUR`);
            }
            if (newSchema.offers && 'price' in newSchema.offers) err(`pk ${t.pk}: state-2a leaked bare Offer.price`);
            if (!fixtures.privateGroup && /groep|group|privé|prive|charter/i.test(cardUnit)) fixtures.privateGroup = { t, oldSchema, newSchema };
        } else {
...
console.log(`draw pool (loadTours filter): ${drawPool.length} of ${rows.length} rows`);
console.log(`emitting population (draw pool + emitPrice): ${pool.length} of ${rows.length} rows; baseline emitted a bare Offer.price on all ${pool.length}`);
console.log('');
...
console.log('');
console.log(`cross-check vs s52 audit: audit ~46 non-per-person rows / ~€18,243 face / 46 with card unit; this census: ${s2n} / ${money(s2face)} / ${tallies['non-per-person-spec'].n}`);
